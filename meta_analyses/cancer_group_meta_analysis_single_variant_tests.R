...
harmonize_effects_to_minor_allele <- function(common_variants) {
  ...
  for (i in seq_along(chromosomes)) {
    ...
    # Rename columns (copy ensures this doesn't affect other chromosomes)
...
for (i in 1:22) {
  chr_UKB_res = fread(paste0("/lindstroem/austin_working/Dissertation/UKBB/gene_based_results/cancer_groups/sparse_GRM/chr",i,"_",group,"_SAIGE_results.txt.singleAssoc.txt")) %>%
    filter(Allele1 != "UR")
  UKB_results = rbind.data.frame(UKB_results,chr_UKB_res)
  chr_AoU_res = fread(paste0("/lindstroem/austin_working/Dissertation/AoU/gene_based_results/cancer_groups/sparse_GRM/chr",i,"_",group,"_SAIGE_results.txt.singleAssoc.txt")) %>%
    filter(Allele1 != "UR")
  AoU_results = rbind.data.frame(AoU_results,chr_AoU_res)
}

UKB_results = merge(UKB_results,
                    UKB_annotations,
                    by.x = "MarkerID",
                    by.y = "Uploaded_variation",
                    all.x = TRUE)

AoU_results = merge(AoU_results,
                    AoU_annotations,
                    by.x = "MarkerID",
                    by.y = "Uploaded_variation",
                    all.x = TRUE)
...
fwrite(meta_results, 
       paste0("/lindstroem/austin_working/Dissertation/Meta/Aim1/cancer_groups/single_meta_",group,".txt"),
       compress = "gzip")
